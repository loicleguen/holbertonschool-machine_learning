#!/usr/bin/env python3
"""Module Yolo - tâche 6 : show_boxes."""
...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        for i, box in enumerate(boxes):
            ...
            label = "{} {:.2f}".format(class_name, box_scores[i])
    ...
    def predict(self, folder_path):
        """Exécute le pipeline complet de détection sur un dossier.
        ...
        # 2. Préprocesser les images pour le modèle
        pimages, image_shapes = self.preprocess_images(images)

        # 3. Prédiction du modèle sur toutes les images en une fois
        outputs = self.model.predict(pimages)

        predictions = []

        for i, image in enumerate(images):
            # 4. Réorganiser les outputs pour cette image
            # outputs est une liste de 3 tableaux de forme
            # (ni, grid_h, grid_w, anchors, 85)
            # on extrait les sorties pour l'image i
            image_outputs = [output[i] for output in outputs]

            # 5. Décoder les boîtes
            ...
            # 8. Afficher l'image avec les boîtes
            # nom de fichier sans le chemin complet
