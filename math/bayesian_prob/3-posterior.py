...
def marginal(x, n, P, Pr):
    """
    Calcule la probabilité marginale d'obtenir les données x et n.
    ...
    ----------
    float
        La probabilité marginale totale d'obtenir les données.
    """
    # L'ordre de validation stricte requis par les consignes
    if not isinstance(n, (int, np.integer)) or n <= 0:
        raise ValueError("n must be a positive integer")
    ...
    # La probabilité marginale est la somme globale de toutes les intersections
    return np.sum(intersection_vals)


def posterior(x, n, P, Pr):
    """
    Calcule la probabilité a posteriori de chaque probabilité dans P
    sachant les données observées x et n.
    ...
    ----------
    np.ndarray
        Un tableau 1D contenant les probabilités a posteriori de P.
    """
    # L'ordre de validation stricte requis par les consignes
    if not isinstance(n, (int, np.integer)) or n <= 0:
        raise ValueError("n must be a positive integer")
    ...
    # Calcul de l'intersection et de la marginale
    inter_vals = intersection(x, n, P, Pr)
    marg_val = np.sum(inter_vals)

    # Règle de Bayes: intersection / marginale
    return inter_vals / marg_val
