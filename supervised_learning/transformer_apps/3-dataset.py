...
"""
Module 3-dataset
Contient la classe Dataset avec le pipeline complet de données TensorFlow.
"""
...
class Dataset:
    """
    Classe Dataset permettant de charger, prétraiter, encoder et construire
    un pipeline performant (filter, cache, shuffle, padded_batch, prefetch)
    ...
    def __init__(self, batch_size, max_len):
        ...
        encode les exemples et configure le pipeline de données.

        Args:
            batch_size: taille des lots (batches)
            max_len: nombre maximal de tokens autorisés par phrase
        """
        ...
        # Encodage initial des phrases
        self.data_train = self.data_train.map(self.tf_encode)
        self.data_valid = self.data_valid.map(self.tf_encode)

        # Filtre sur la longueur maximale max_len
        ...
        # Configuration du pipeline de données d'entraînement
        ...
        # Configuration du pipeline de données de validation
